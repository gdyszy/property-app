<template>
    <view class="mpvue-picker">
        ...
        <view class="mpvue-picker-content " :class="{'mpvue-picker-view-show':showPicker}">
			...
			<picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='dateMonthSelector'">
			    <block>
			        <picker-view-column>
			            <view class="picker-item" v-for="(item,index) in pickerValueYears" :key="index">{{item.label}}</view>
			        </picker-view-column>
            ...
            <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='yearSelector'">
                <block>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index) in pickerValueYears" :key="index">{{item.label}}</view>
                    </picker-view-column>
            ...
            <picker-view indicator-style="height: 40px;" class="mpvue-picker-view" :value="pickerValue" @change="pickerChange" v-if="mode==='monthSelector'">
                <block>
                    <picker-view-column>
                        <view class="picker-item" v-for="(item,index) in pickerValueMonths" :key="index">{{item.label}}</view>
                    </picker-view-column>
        ...
        watch: {
			...
			pickerValue: {
				handler(val) {
					...
					const pickerMon = val[1] + 1;
					let Days = [];
					let isBigMon = BigMonths.includes(pickerMon);
					let dayCnt = isBigMon ? 31 : 30;
        ...
        methods: {
            ...
            _getPickerLabelAndValue(value, mode) {
                ...
                if (mode === 'selector') {
					...
					pickerLable = `${this.pickerValueYears[value[0]].value}-${this.pickerValueMonths[value[1]].value}`;
					pickerGetValue.push(this.pickerValueYears[value[0]].value);
					pickerGetValue.push(this.pickerValueMonths[value[1]].value);
				}else if(mode === 'yearSelector'){
					pickerLable = `${this.pickerValueYears[value[0]].value}`;
					pickerGetValue.push(this.pickerValueYears[value[0]].value);
				}else if(mode === 'monthSelector'){
					pickerLable = `${this.pickerValueMonths[value[0]].value}`;
